...
##This tool will map RNAseq samples with Kallisto.
...
while getopts "h?C:f:o:s:u:" option; do
    case "$option" in
     h|\?)
          ...
          echo "-f, --Master-file                     specify the MasterFile.txt (info run)"
      ...
      f)
          Master=$OPTARG
          if [ ! -f $Master ]; then
              echo "-f: Master File does not exist"
              exit 1
          fi
          ;;
      o)
...
if [ $Cores != $Cores -o $Master != $Master -o $OUT_FOLDER != $OUT_FOLDER -o $strandness != $strandness -o $UsefulData != $UsefulData ]; then
    echo 'ERROR: [-C] [-f] [-o] [-s] [-u] are required'
fi



#PATH
export PATH=/hpcnfs/software/anaconda/anaconda3/bin:$PATH

KALLISTO_path=$UsefulData/DB/iGenome/Homo_sapiens/UCSC/hg38/Sequence/KALLISTOindex/
gencode_path=$UsefulData/DB/iGenome/Homo_sapiens/UCSC/hg38/Annotation/hg38_gencode_annotation/
Chr_size=$UsefulData/DB/Chr_size_info/
