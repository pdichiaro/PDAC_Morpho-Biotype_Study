...
|   └─ RNAseq.LMD.txi.import.R: Script for the generation of a reference database
...
|   └─ viper_script.R: Analysis code to infer master regulators using VIPER tool
|   └─ session_R_info.txt: txt file collecting info of R session related to viper_script.R
|   └─ Giggle/:
|   |   └─ promoters_targets_giggle.R: Script for the extraction of promoter sequences of the targets of master regulators
|   |   └─ Giggle.sh: Script to query input promoter regions against the genomic index and evaluate enrichment patterns
└─ 4. scRNAseq/:
